...
public class GeneticAlgorithmObjectIdentificationMethodWithoutSampling implements
		IObjectIdentificationMethod {

	private FusionMethodWrapper descriptor;
	private static Logger log = Logger.getLogger(GeneticAlgorithmObjectIdentificationMethodWithoutSampling.class);
	...
	public GeneticAlgorithmObjectIdentificationMethodWithoutSampling() {
	...
	public List<AtomicMapping> getAllPossibleMappings(ApplicationContext context) throws FusionException {
		...
		try {
			...
			Map<Integer, Double> finalResults = candidateSolutionPool.run(false);
	...
	private List<AtomicMapping> createAtomicMappings(Map<Integer, Double> resultsEncoded, MemoryInstanceCache cache, CandidateSolution solution) {
		...
		for(Integer id : resultsEncoded.keySet()) {
			...
			mappings.add(pair.convertToAtomicMapping(solution.getModelSpec(), resultsEncoded.get(id), solution.getFitness().getPrecision()));
		}
